/*
 on service start...
  ...
  on interval
    ...
    hold a struct (most like a dictionary) for each worker/subaccount/user/coin/account
  ...
  workers:
    ...
    after the looping
      ...
      if the share count < predetermined value... set hashrate to 0 then blend
      if the last share time - initial share time < predetermined... set hashrate to 0 then blend
      in the dict, save an array of hashrates

    after calcing the hashrate
      potentially, store it in another struct , with the same keys as the workers struct and blend
